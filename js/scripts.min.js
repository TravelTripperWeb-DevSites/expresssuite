 ...
 * Build: http://modernizr.com/download/#-opacity-cssanimations-csstransforms-csstransforms3d-csstransitions-svg-touch-printshiv-cssclasses-teststyles-testprop-testallprops-prefixes-domprefixes-css_backgroundsizecover
 */
...
$(document).ready(function () {
	...
	$.each(poiInstances, function(){
		...
		var poi = $(this),
			...
			categories = $(".poi-category", categoriesContainer),
			descriptions = $(".description", categories),
			hotels = $('.hotel', poi),
			markerTypes = {
				regular: new google.maps.MarkerImage(templateURL + 'assets/parallax/images/pin-small.png', null, null, null, new google.maps.Size(33, 47)),
				hotel: new google.maps.MarkerImage(templateURL + 'assets/parallax/images/pin.png', null, null, null, new google.maps.Size(44, 63))
			};

		if (categories.length) {
			
			$('.text', categorySelector).text( $('.button.active', categorySelector).text() ).click(function(event){
			...
			categorySelectorButtons.click(function(){
				...
				button.parent().removeClass('visible');
				button.parent().siblings('.text').text( button.text() );
			...
			categories.each(function(){
				...
				points.each(function(){
					...
					var markerIcon = markerTypes.regular;
					if (point.data('icon')) {
						markerIcon = templateURL + 'assets/parallax/images/pins/'+point.data('icon')+'.png';
					}

					$.when( $.get(markerIcon) ).done(function() {
						var marker = new google.maps.Marker({
							position: new google.maps.LatLng(point.data('lat'), point.data('lng')),
							icon: new google.maps.MarkerImage(markerIcon, null, null, null, new google.maps.Size(30, 51)),
							map: map
						});
						var infoContent = '<div class="map-content"><h4>' + point.data('name') + '</h4><p>' + point.data('address') + (point.data('phone') ? '<br>Phone: ' + point.data('phone') : '') + '</p><p><a href="http://maps.google.com/maps?f=d&amp;geocode=&amp;daddr=' + point.data('lat') + '%2C%20' + point.data('lng') + '&amp;z=16" class="button" target="_blank">Get directions</a></p></div>';
...
// @codekit-prepend "_modernizr.js", "_jquery.ui.js", "_sselect.js", "_validation.js", "_validation-en.js", "_bxslider.js", "_poi.js", "_galleria.js", "_jquery.scrollTo.js", "_cookie.js";
...
$(function(){
	...
	if(window.location.search=="?pop=done") popUpSubmitted=true;

	//show pop-up if user has not already signed up
	if( $('#popup-wrapper').length && $.cookie('popupSignup') != 'signedUp' && !popUpSubmitted){
		$.getJSON('/travel-agent-rewards/popup?format=json', {content:[0]}, function(data){
			
		    var content = data.content;
		    content=content.split('<p></p><div></div>').pop();
		    
		    if(content) {
		        $('#popup-wrapper .inner-popup').append(content);
		        $('html').addClass('popup');
		    }
		    else {
		        $('#popup-wrapper').remove();
		        
				displayTravelRewardsSignup();
		    }
		})
			.done(function(){
				//Popup - set input placeholder
				$('#travel_agent_rewards_popup input[type=text]').each(function(){
					var currentInput = $(this);
					var inputID = currentInput.attr('id');
					var labelName = $('#travel_agent_rewards_popup label[for='+inputID+']');
					labelName.find('em').remove();
					currentInput.attr('placeholder', labelName.html());
				});

				var popupForm = $('#popup-wrapper form');

				popupForm.on('submit', function(){
				
					if( popupForm.validationEngine('validate') ){
						$('html').removeClass('popup');
						$.cookie('popupSignup', 'signedUp', { expires: 999 });
					}
				});
	...
	function displayTravelRewardsSignup() {
		$.getJSON('/travel-agent-rewards/travel-agent-form?format=json', {content:[0]}, function(data){
			
		    var content = data.content;
		    content=content.split('<p></p><div></div>').pop();
		    
		    if(content) {
		        $('.travel-agent-rewards-program-section .page-content').append(content);
		        if($.validationEngine) $('#travel_agent_rewards').validationEngine();
		        $('#travel_agent_rewards select').sSelect();
		    }
		});
	}
	...
	var windowObject = $(window),
		...
		header = $('#header'),
		photos = $('#photos'),
		contentContaier = $('#content'),
		promos = $('#parallax-promos'),
		...
		function setDeviceSize(w) {
			...
			deviceSize = (w >= 1024) ? 'desktop' : (w < 768) ? 'mobile' : 'tablet';
			rootObject.addClass(deviceSize);
		};
		
		setDeviceSize(windowObject.width());
		
		windowObject.on('resize', function() {
			setDeviceSize($(this).width());
		});
	...
	$('#main-menu a[data-anchor]').each(function(){
		$(this).attr('href', rootURL + '#' + $(this).data('anchor'));
	}).click(function(){
		var hash = $(this).data('anchor');
		updateLocationHash(hash);	
		
		rootObject.removeClass('show-menu-container');
		
		var headerHeight = (header.height() > 180) ? 0 : header.height();
		
		$('html, body').stop().animate({scrollTop: $('#' + hash).offset().top}, 400);
		return false;
	});
	...
	if (!isMobile.any()) {
		...
		var processCheckinCheckoutInputs = function(checkin, checkout, prefill, interval){
			...
			if (prefill){
				...
				if (checkout) checkout.datepicker("setDate", defaultCheckoutDate);
		...
		processCheckinCheckoutInputs($("#checkin"), $("#checkout"), true, true);
	} else {
		rootObject.addClass('mobile-device');
		$('.date-pick').attr('type', 'date');
	}

	// STYLISH SELECT

	$('#booking select, .page-content .form select, #calendar-filter select').sSelect();
	
	// scroll to explore
	$('.scroll-to-explore', photos).click(function(event){
		event.preventDefault();
		$('html, body').stop().animate( { scrollTop: $('#overview-section').offset().top}, 400);
	});
	...
	var state = deviceSize;
	
	if ($('.parallax-promo-item', promos).length <= 3) {
		rootObject.addClass('promotiles-'+$('.parallax-promo-item', promos).length);
	}
	
	if($('.parallax-promo-item', promos).length > 1) {
		var currentScroll = 0,
			scrolled = 0,
			itemWidth = $('.parallax-promo-item', promos).outerWidth(),
			itemsTotal = $('.parallax-promo-item', promos).length,
			scrolledCounter = 0;
			
			$('.slideshow', promos).on('scroll', function(e){
				itemWidth = $('.parallax-promo-item', promos).outerWidth();
				scrolled = $(this).scrollLeft();
				
				//normalize
				scrolled = Math.min(scrolled, itemsTotal * itemWidth);
				scrolled = Math.max(scrolled, 0);
				
				// console.log('Difference: ' + Math.abs(currentScroll - scrolled));
				
				console.log('Current Scroll: ', currentScroll, 'Scrolled: ', scrolled, 'Diff: ', Math.abs(currentScroll - scrolled))
				
				if( Math.abs(currentScroll - scrolled) >= itemWidth ) {

					//scrolledCounter = (scrolled >= currentScroll) ? (scrolledCounter + 1)  : (scrolledCounter - 1);

					currentScroll = itemWidth * Math.round(scrolled/itemWidth);
					
					//normalize
					currentScroll = Math.min(currentScroll, itemsTotal * itemWidth);
					currentScroll = Math.max(currentScroll, 0);
					
					scrolledCounter = parseInt(currentScroll/itemWidth);
					
					// console.log('Scrolled Counter After: ', scrolledCounter);
					// $('#pager li').removeClass('active').filter('[data-id="' + scrolledCounter + '"]').addClass('active');					
			...
			$('#arrows a').on('click', function(e){
				...
				var toItem = ($(this).hasClass('next')) ? Math.min(scrolledCounter+1, itemsTotal - 1) : Math.max(0, scrolledCounter-1);
				// console.log('To Item: ', toItem, 'Scrolled Counter Before: ', scrolledCounter);

				$('.slideshow', promos).scrollTo($('.parallax-promo-item', promos).filter('[data-id="' + toItem + '"]'), 500, {axis: 'x', onAfter: function() {
					scrolling = false;
				}});
...
// 			pause: 6000,
// 			speed: 600,
// 			minSlides: (deviceSize == "desktop") ? 3 : (deviceSize == "tablet") ? 2 : 1,
// 			maxSlides: (deviceSize == "desktop") ? 3 : (deviceSize == "tablet") ? 2 : 1,
// 			moveSlides: 1,
// 			slideWidth: (deviceSize == "desktop") ? 400 : (deviceSize == "tablet") ? 370 : 320,
// 			pager: (deviceSize == "desktop") ? false : true
// 		}
		
// 		var promoSlider = $('.slideshow', promos).bxSlider(promoBxOptions);
...
// //					MOBILE SIZE
// 					promoBxOptions.minSlides = 1;
// 					promoBxOptions.maxSlides = 1;
// 					promoBxOptions.slideWidth = 320;
// 					promoBxOptions.pager = true;
// 				} else if( deviceSize == 'tablet' ) {
// //					TABLET SIZE
// 					promoBxOptions.minSlides = 2;
// 					promoBxOptions.maxSlides = 2;
// 					promoBxOptions.slideWidth = 370;
// 					promoBxOptions.pager = true;
// 				} else {
// //					DESKTOP SIZE
// 					promoBxOptions.minSlides = 3;
// 					promoBxOptions.maxSlides = 3;
// 					promoBxOptions.slideWidth = 400;
// 					promoBxOptions.pager = false;
// 				}
				
// 				state = deviceSize;
				
// 				promoSlider.reloadSlider(promoBxOptions);
// 			}
...
// 	}
	...
	if ($.validationEngine) {
		$(".page-content .form").validationEngine({promptPosition: "topLeft", autoPositionUpdate: true, scroll: false});
		$("#booking .form").validationEngine({promptPosition: "topLeft", autoPositionUpdate: true, scroll: false});
		$("#newsletter form").validationEngine({promptPosition: "topLeft", autoPositionUpdate: true, scroll: false});
	}
	...
	$('.items-grid').each(function(){
		var itemsGrid = $(this),
			categorySelector = $('.category-selector', itemsGrid),
			items = $('.item', itemsGrid);

		$('.button', categorySelector).click(function(event){
			var button = $(this);

			button.addClass('active').siblings().removeClass('active');
			button.parent().removeClass('visible');
			// CHANGE PSEUDO SELECT NAME
			button.parent().siblings('.text').text( button.text() );
			
			if( button.data('category-id') != 'all' ) {
				items.removeClass('active').filter('[data-category-id="' + button.data('category-id') + '"]').addClass('active');
			} else {
				items.addClass('active');
			}
			
			$('.items', itemsGrid).attr('class', 'items').addClass('w' + items.filter(".active").length);
			// console.log(items.filter(":visible").length);
			
			// if(eventsSlider != 'undefined' || eventsSlider != null) {
			// 	eventsSlider.destroySlider();
			// 	cloneActiveEvents();
			// 	if( $('.item', eventsSlider).length > 2 ) {
			// 		eventsSlider.bxSlider(eventsBxOptions);
			// 	}
			// }
		...
		$('.text', categorySelector).click(function(event){
			$(this).siblings('.options').toggleClass('visible');
		});
	});
	...
	$('#menu-switch').on('click', function(event){
		if(rootObject.hasClass('show-menu-container')) {
			rootObject.removeClass('show-menu-container');
		} else {
			rootObject.addClass('show-menu-container');
			
			documentObject.on('click.menu', function(event) {
				if (!$(event.target).parents('#main-menu').length) {
					rootObject.removeClass('show-menu-container');
					documentObject.off('click.menu');
				}
			});
	...
	$('#book-switch').on('click', function(){
		rootObject.addClass('show-booking-container');
	});
	$('#book-close').on('click', function(){
		rootObject.removeClass('show-booking-container');
	});
	...
	var headerHeight = header.height(),
		photosMinHeight = 300,
		photosMaxHeight = 756,
		contentPreviewHeight = 60;

	var windowChangeHandler = function(){
		
		var newPhotosHeight = photosMinHeigth;

		newPhotosHeight = windowObject.height() - headerHeight - contentPreviewHeight;
		if (newPhotosHeight < photosMinHeight) newPhotosHeight = photosMinHeight;
		if (newPhotosHeight > photosMaxHeight) newPhotosHeight = photosMaxHeight;

		photos.height(newPhotosHeight);
	};

	// $(window).on('resize', windowChangeHandler);
	// windowChangeHandler();
