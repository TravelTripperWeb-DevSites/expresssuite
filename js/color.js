...
*/
var CanvasImage = function (image,k) {
	...
	tw = Math.floor(imgEl.width/10);
	th = Math.floor(imgEl.height/10);

	this.canvas.width = tw;
	this.canvas.height = th/k;

    this.context.drawImage(imgEl, 0, 0, this.width, this.height/k, 0, 0, tw, th/k);
};
...
function getDominantColor(sourceImage) {
    ...
    var palette = createPalette(sourceImage, 5, 4);
 ...
 */
function createPalette(sourceImage, colorCount, k) {

    // Create custom CanvasImage object
    var image = new CanvasImage(sourceImage, k),
